...
def get_dataset_path(dataset_name):
    return FILES_DIRECTORY.joinpath(dataset_name)
...
def create_on_disk(dataset_name, zip_file):
    ...
    with zipfile.ZipFile(zip_file) as zf:
        # TODO: handle unexpected files/file contents
        for zip_info in zf.infolist():
            if zip_info.filename.rsplit('.', 1)[-1] != 'csv':
                return False
...
def create_on_mongo(dataset_name, client):
    dataset_path = get_dataset_path(dataset_name)

    client[DATABASE][DATASETS_COLLECTION]
